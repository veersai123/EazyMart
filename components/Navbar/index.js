...
function NavItems() {
    const navItems = isAdminView ? adminNavOption : navOptions;

    return (
        <div className="items-center justify-between w-full md:flex md:w-auto" id="nav-items">
            <ul className="flex flex-com p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg md:flex-row md:space-x-8 md:mt-0 md:border-0 bg-white">
                {navItems.map(item => (
            ...
            <nav className="bg-white fixed w-full z-20 top-0 left-0 border-b border-gray-200">
                <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4" >
                    ...
                    <div className="flex md:order-2 gap-2">
                        ...
                        {user?.role === 'admin' &&
                            <button className={styles.button}>{isAdminView ? 'Client View' : 'Admin View'}</button>
                        }
